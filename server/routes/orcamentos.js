...
router.put('/:id', requireAuth, (req, res) => {
    ...
    const { cliente_id, cliente_nome, projeto, ambiente, ambientes, mods, taxas, padroes, pagamento, obs, custo_material, valor_venda, status, kb_col, numero, data_vencimento, prazo_entrega, prazo_execucao, endereco_obra, validade_proposta, validade_dias, arquiteta_nome, portal_ativo, portal_validade_ate } = req.body;
    ...
    UPDATE orcamentos SET
      ...
      numero=?, data_vencimento=?, arquiteta_nome=?, portal_ativo=?, portal_validade_ate=?,
  ...
  `).run(
        ...
        portal_ativo !== undefined ? (portal_ativo ? 1 : 0) : (existing.portal_ativo ?? 1),
        portal_validade_ate !== undefined ? (portal_validade_ate || null) : (existing.portal_validade_ate || null),
        id
    );
